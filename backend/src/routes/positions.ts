...
export async function positionsRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/api/positions', { preHandler: authMiddleware }, async (request, reply) => {
    ...
    try {
      let query = `SELECT * FROM trading_positions WHERE user_id = $1`;
    ...
    async (request, reply) => {
      ...
      try {
        const result = await client.query<TradingPosition>(
          'SELECT * FROM trading_positions WHERE id = $1 AND user_id = $2',
          [id, userId]
        );
    ...
    try {
      ...
      const feedbackResult = await client.query(
        `SELECT
         ...
         FROM trading_positions
         WHERE user_id = $1 AND opened_at >= $2 AND user_feedback IS NOT NULL
